...
class Vision:
    ...
    def __init__(self, needle_img_path, method=cv.TM_CCOEFF_NORMED):
        ...
        cv2.imshow('needle', self.needle_img)
...
vision_red_candy = Vision('OPENCV_tutorial_tests/candies/green/green.png', method=cv.TM_CCOEFF_NORMED)

vision_red_candy.init_control_gui()

while True:

    screen = wincap.get_screenshot()
    processed_image = vision_red_candy.apply_hsv_filter(screen)

    red_rectangles = vision_red_candy.find(processed_image, 0.5)
    # orange_rectangles = vision_orange_candy.find(processed_image, 0.5)
    # yellow_rectangles = vision_yellow_candy.find(processed_image, 0.5)
    # green_rectangles = vision_green_candy.find(processed_image, 0.5)
    # blue_rectangles = vision_blue_candy.find(processed_image, 0.5)
    # purple_rectangles = vision_purple_candy.find(processed_image, 0.5)

    rectangles = red_rectangles

    out_image = vision_red_candy.draw_rectangles(screen, rectangles)
