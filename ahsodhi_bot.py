...
candy_actions, candy_sensor, candy_agent = init_bot(game_name, game_engine)
...
b_data = candy_sensor.get_board_data()
y_offset = 19
...
VRANGE_SC = cell_size_h
HRANGE_SC = cell_size_w
    ...
    if (direction == LEFT):
        ...
        while (i <= c):
            ...
            if not board[r][i] in seen:
                ...
                matches[list(matches.keys())[seen.index(board[r][i])]] += 1
            i += 1
        if (sequence-1 in matches.values() and 1 in matches.values() and len(matches.keys()) == 2):
            target = list(matches.keys())[list(matches.values()).index(sequence-1)][0]
            C = list(matches.keys())[list(matches.values()).index(1)][1]
            if (C == c):
                restrict = (1,0,0,0)
            elif (C == c-(sequence-1)):
                restrict = (0,1,0,0)
    ...
    if (direction == TOP):
        ...
        while (i >= r-(sequence-1)):
            ...
            if not board[i][c] in seen:
                ...
                matches[list(matches.keys())[seen.index(board[i][c])]] += 1
            i -= 1
        if (sequence-1 in matches.values() and 1 in matches.values() and len(matches.keys()) == 2):
            target = list(matches.keys())[list(matches.values()).index(sequence-1)][0]
            R = list(matches.keys())[list(matches.values()).index(1)][1]
            if (R == r):
                restrict = (0,0,1,0)
            elif (R == r-(sequence-1)):
                restrict = (0,0,0,1)
...
def initializeBoard(gameboard):
    image = wincap.get_screenshot(b_data[0], b_data[1]-y_offset, b_data[2]*9, b_data[3]*9)
